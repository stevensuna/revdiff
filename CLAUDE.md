...
- When bumping at release, keep each marketplace entry synchronized with its plugin manifest. For `revdiff-planning`, update root `plugin.json`, both compatibility manifests, and its version in `.claude-plugin/marketplace.json`.
